...
public class DestroyNpc2 : MonoBehaviour
{
    // Start is called before the first frame update
   [SerializeField] private GameObject prefab;
   [SerializeField] private GameObject geser;
    ...
    void Update()
    {
        if (Input.GetMouseButtonDown(0))
        {
            ...
            if (Physics.Raycast(ray, out hit)){
                ...
                if (hit.collider.CompareTag("kate")) {
                    Destroy(hit.transform.gameObject);
                    Instantiate(geser, hit.collider.transform.position, Quaternion.identity);
                    Debug.Log("Clicked on " + hit.transform.gameObject.name);
