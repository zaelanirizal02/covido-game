...
public class DestroyNpc : MonoBehaviour
{
   ...
   [SerializeField] private GameObject bossX;
      [SerializeField] private GameObject remyX;
      [SerializeField] private GameObject kateX;
    ...
    void Update()
    {
        if (Input.GetMouseButtonDown(0))
        {
            ...
            if (Physics.Raycast(ray, out hit)){
                if (hit.collider.CompareTag("bossX")) {
                    Destroy(hit.transform.gameObject);
                    Instantiate(bossX, hit.collider.transform.position, Quaternion.identity);
                    Debug.Log("Clicked on " + hit.transform.gameObject.name);
                    
                    // 19,94 0.18 23,62
                }

                if (hit.collider.CompareTag("remyX")) {
                    Destroy(hit.transform.gameObject);
                    Instantiate(remyX, hit.collider.transform.position, Quaternion.identity);
                    Debug.Log("Clicked on " + hit.transform.gameObject.name);
                    
                    // 19,94 0.18 23,62
                }

                 if (hit.collider.CompareTag("kateX")) {
                    Destroy(hit.transform.gameObject);
                    Instantiate(kateX, hit.collider.transform.position, Quaternion.identity);
                    Debug.Log("Clicked on " + hit.transform.gameObject.name);
